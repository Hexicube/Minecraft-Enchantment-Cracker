...
namespace Minecraft_Enchantment_Cracker
{
    public class ItemBackgroundCanvas : Canvas
    {
        ...
        protected override void OnRender(DrawingContext dc)
        {
            ...
            for(int y = 0; y < MainWindow.avail.Length; y++)
            {
                ...
                {
                    dc.DrawRectangle(
                        MainWindow.avail[y][x]
                            ? ((x == MainWindow.ItemSelection.x && y == MainWindow.ItemSelection.y)
                                ? brushSEL
                                : brushOK)
                            : brushBAD,
                        null,
                        new Rect(x * 36, y * 36, 36, 36)
                    );
                }
            }
        }
    ...
    public partial class MainWindow : Window
    {
        ...
        public static bool[][] avail =
            MinecraftData.GetAvailability(MainWindow.MinecraftVersion);
            ...
            if(e.ChangedButton == MouseButton.Left)
            {
                ...
                else if(p.Y < 58)
                {
                    ...
                    if(tab != CurrentTab)
                    {
                        ...
                        CurrentTab = tab;
                        if(tab == 0)
                            ProgressTask = SeedCracker;
                        else if(tab == 1)
                            ProgressTask = ItemFinder;
                        else
                            ProgressTask = Version;
                    }
                }
            }
        ...
        public static bool IsNumeric(string text) { return text.All(c => Char.IsDigit(c)); }

        private void TextEntryNumeric(object sender, TextCompositionEventArgs e) { e.Handled = !IsNumeric(e.Text); }

        private void TextPasteNumeric(object sender, DataObjectPastingEventArgs e)
        {
            if(e.DataObject.GetDataPresent(typeof(string)))
            {
                if(!IsNumeric((string)e.DataObject.GetData(typeof(string))))
                    e.CancelCommand();
            }
            else
                e.CancelCommand();
